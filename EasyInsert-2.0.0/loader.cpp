...
////////////////////////////////////////////////////////////
// My functions
std::string renameFrameName(std::string frameName, std::string addition) {
    //return addition + "." + frameName;
    return addition + "." + frameName;
}
...
void addDevicePropsToFrame(DummyDevice &dev, const std::string& name, DummySetup &setup) {
	...
	for (size_t j = 0; j < modellist.size(); j++) {
		addModelToFrame(modellist[j], frame, setup.tree, setup); // HER TILFØJES MODELLER TIL FRAMES I ET DEVICE
	}
        ...
        if (name != "") {
            ...
            for (int i = 0; i < setup.dwc->_devlist[0]._frames.size(); i++) {
                ...
                if (setup.dwc->_devlist[0]._frames[i]._refframe != "WORLD")
                    setup.dwc->_devlist[0]._frames[i]._refframe = renameFrameName(setup.dwc->_devlist[0]._frames[i]._refframe, name); // fix refframe
...
void ei::loader::add(std::string filename, rw::models::WorkCell::Ptr wc, std::string name, std::string startFrame, double x, double y, double z, double R, double P, double Y) {
    ei::loader::add(filename, wc, name, startFrame, ei::creator::getTransform3D(x, y, z, R, P, Y));
}
